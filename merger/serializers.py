...
class PdfFileSerializer(serializers.Serializer):
    ...
    def validate_file(self, file):
        if file.content_type != "application/pdf":
            raise serializers.ValidationError(f"error: '{file.name}' is not a PDF file. Please make sure to upload only PDF files.")
        ...
        if file.size > size_limit:  
            raise serializers.ValidationError(f"error: '{file.name}' file exeeds size limit.")
        return file
